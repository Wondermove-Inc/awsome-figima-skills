...
3. **Spacing is rhythm — the gap encodes the relationship, and the wrong gap is the #1 craft tell.** Space
   ...
   than inner** (use `rad-spacing`). Match **density to the content and platform** — a data-dense list is
...
> nothing). Instead read **PROGRESSIVELY — just-in-time, one skill per phase**: the cheat-sheet first, then
> `figma-design-patterns` + `rad-spacing` when you start composing layout & spacing (**before** you set
> gaps/padding — the phase builds go weak at), `frontend-design` during polish, `figma-mcp-express` the
> moment a tool/op question arises. Read → apply → move on. The two failures to avoid are equal: the
> upfront firehose AND never opening the spacing/layout skill at all. Full-load-every-run (below) is the
> single-screen `/figma-redesign` default; the product brief overrides it with this progressive ladder.
...
- **`figma-design-patterns`** — auto-layout, component-first, states, dark mode, spacing, handoff: the
  shared craft rules you, your self-eval, and the reviewer all judge against.
- **`frontend-design`** — production-grade interface quality (the design sense above, operationalized).
- **`rad-spacing`** — hierarchical Gestalt spacing from the file's library spacing variables.
...
These hold no matter what; `figma-mcp-express` + `figma-design-patterns` give the exact tool calls. Use
...
## DESIGN GUARDRAILS — frontend-design anti-patterns (NEVER use)

Apply the frontend-design skill's design thinking to every section you build. Specifically avoid:
...
- `backend`: `"official"` | `"figma-go"` | `"mixed"` — selects execution path (default: `"official"`)
